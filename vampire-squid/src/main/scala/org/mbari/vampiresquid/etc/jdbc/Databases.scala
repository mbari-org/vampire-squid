...
object Databases:
     ...
     * @param url The JDBC URL to infer the type from.
     * @return The inferred DatabaseType.
     */
    def typeFromUrl(url: String): DatabaseType =
        if url.contains("postgresql") then
            DatabaseType.PostgreSQL
        else if url.contains("oracle") then
            DatabaseType.Oracle
        else if url.contains("sqlserver") then
            DatabaseType.SQLServer
        else
            throw new IllegalArgumentException(s"Unknown database type for URL: $url")
