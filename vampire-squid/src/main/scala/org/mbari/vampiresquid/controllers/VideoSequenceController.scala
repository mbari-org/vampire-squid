...
class VideoSequenceController(val daoFactory: JPADAOFactory) extends BaseController:
    ...
    private def execReadOnly[T](fn: VSDAO => T)(implicit ec: ExecutionContext): Future[T] =
        val dao = daoFactory.newVideoSequenceDAO()
