...
class VideoSequenceEndpoints(controller: VideoSequenceController)(using ec: ExecutionContext, jwtService: JwtService) extends Endpoints:
  ...
  val findLastUpdateEndpointImpl: ServerEndpoint[Any, Future] = 
    findLastUpdateEndpoint
      .serverLogic { req => handleOption(controller.findByUUID(req).map(opt => {
        for 
          vs <- opt
  ...
  val findByCameraIdAndTimestampEndpointImpl: ServerEndpoint[Any, Future] =
    findByCameraIdAndTimestampEndpoint
      .serverLogic { (cameraId, timestamp) => handleErrors(controller.findByCameraIDAndTimestamp(cameraId, timestamp).map(_.toList)) }
  ...
  val createEndpointImpl: ServerEndpoint[Any, Future] = 
    createEndpoint
      ...
      .serverLogic(_ => req => {
        ...
        if (name.isEmpty || cameraId.isEmpty) Future.successful(Left(BadRequest("Missing name or camera_id")))
        else {
