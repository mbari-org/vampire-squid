...
class OracleSuite extends munit.FunSuite:
  ...
  test("Oracle init script should have been run"):
    val em = daoFactory.entityManagerFactory.createEntityManager()
    val q  = em.createNativeQuery("SELECT COUNT(*) FROM unique_videos")
    val r = q.getResultList().asScala.toList.head.asInstanceOf[java.math.BigDecimal].intValue()
    println("---------------------" + r)
    assert(r == 0)