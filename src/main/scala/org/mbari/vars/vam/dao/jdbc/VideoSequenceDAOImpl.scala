...
/**
  ...
  */
class VideoSequenceDAOImpl(val connection: Connection) extends VideoSequenceDAO {
  ...
  override def findByTimestamp(timestamp: Instant): Iterable[VideoSequence] = ???
  ...
  override def create(entity: VideoSequence): VideoSequence = ???
