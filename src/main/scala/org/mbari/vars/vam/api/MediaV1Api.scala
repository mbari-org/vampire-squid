...
class MediaV1Api(controller: MediaController)(implicit val swagger: Swagger, val executor: ExecutionContext)
  ...
  get("/sha512/:sha512") {
    ...
    controller.findBySha512(sha).map({
      case None => halt(NotFound("""{not_found: "A video with matching checksum was not found"}"""))
  ...
  get("/videoreference/:uuid") {
    ...
    controller.findByVideoReferenceUuid(uuid)
      .map({
        case None => halt(NotFound(s"""not_found: "A media with a video-reference uuid of $uuid was not found"}"""))
        case Some(v) => controller.toJson(v)
      })
  }
  ...
  get("/camera/:camera_id/:start_time/:end_time") {
    ...
    val startTime = params.getAs[Instant]("start_time")
      .getOrElse(halt(BadRequest("""{error: "A 'start_time' parameter in UTC time formated as 'yyyy-mm-ddThh:MM:ssZ' is required"}""")))

    val endTime = params.getAs[Instant]("end_time")
      .getOrElse(halt(BadRequest("""{error: "An 'end_time' parameter in UTC time formated as 'yyyy-mm-ddThh:MM:ssZ' is required"}""")))
  ...
  get("/camera/:camera_id/:datetime") {
    ...
    val ts = params.getAs[Instant]("datetime")
      .getOrElse(halt(BadRequest("""{error: "An 'datetime' parameter in UTC time formated as 'yyyy-mm-ddThh:MM:ssZ' is required"}""")))
  ...
  get("/uri/:uri") {
    ...
    controller.findByURI(uri).map({
      case None => halt(NotFound(s"""{not_found: "A video with a url matching $uri was not found"}"""))
      case Some(v) => controller.toJson(v)
    })
