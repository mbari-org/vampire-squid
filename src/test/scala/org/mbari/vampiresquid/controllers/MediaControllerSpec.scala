...
import org.mbari.vampiresquid.controllers.{MediaController, VideoReferenceController, VideoSequenceController}
...
class MediaControllerSpec extends AnyFlatSpec with Matchers with BeforeAndAfterEach:
  ...
  it should "update" in:
    ...
    m1.startTimestamp.getEpochSecond should be(m0.startTimestamp.getEpochSecond)
...
//    println(s"${Constants.GSON.toJson(m1p)}")
    ...
    m0d.startTimestamp.getEpochSecond should be (m0a.startTimestamp.getEpochSecond)
